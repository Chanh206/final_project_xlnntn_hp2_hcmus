...
Chế độ 1 — hàng loạt từ file danh sách link volume (mỗi dòng 1 link, có/không /page/N):
  python scripts/download_nomfoundation_images.py \
      --links-file data/input/link.txt \
      --out-root data/input

  Với mỗi link, script tự mở trang "page 1" của volume đó để dò mã ảnh nội bộ
  (ví dụ: nlvnpf-0501) và tổng số trang, rồi tải hết vào data/input/HVH_<NNN>/.
  <NNN> đánh số theo thứ tự dòng trong links-file (dòng 1 -> HVH_001, dòng 2 -> HVH_002, ...).
  Tên file: HVH_001_0001.jpg, HVH_001_0002.jpg, ... (0001 = trang 1, 0002 = trang 2, ...).
...
def download_volume(image_code: str, total_pages: int, prefix: str, out_dir: Path,
    ...
    for i, page in enumerate(tqdm(range(start, start + total_pages), desc=prefix, leave=False)):
        dest = out_dir / f"{prefix}_{i + 1:0{width}d}.jpg"
...
def run_batch(links_file: Path, out_root: Path, quality: str, delay: float) -> None:
...
def main():
    ...
    parser.add_argument("--prefix", help="Tiền tố tên file, ví dụ: HVH_001 (chế độ 1 volume thủ công)")
    ...
    if args.links_file:
        run_batch(Path(args.links_file), Path(args.out_root), args.quality, args.delay)
        return
